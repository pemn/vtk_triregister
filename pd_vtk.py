...
class vtk_Voxel_(object):
  ...
  def from_bb_schema(cls, bb, schema, ndim = 3):
    ...
    offset = 0
    s = schema.split(';')
    cell_size = np.asfarray(s[0].split(','))
    if len(s) > 1:
      bearing = float(s[1])
    if len(s) > 2:
      offset = np.asfarray(s[2].split(','))
    if len(cell_size) < 3:
      cell_size = np.resize(cell_size, 3)
    
    if bearing != 0:
      mesh = pv.PolyData(bb, deep=True)
      mesh_rotate_z_0261(mesh, bearing, mesh.center)
      bb_r = np.transpose(np.reshape(mesh.bounds, (3,2)))

      bb[0] = np.min([bb[0], bb_r[0]], 0)
      bb[1] = np.max([bb[1], bb_r[1]], 0)

    if offset != 0:
      bb[0] = np.add(bb[0], np.multiply(cell_size, np.add(offset, -1)))
      bb[1] = np.add(bb[1], np.multiply(cell_size, offset))

    self = cls.from_bb(bb, cell_size, ndim)

    if bearing != 0:
      self = vtk_VoxelSG(self.cast_to_structured_grid())
      mesh_rotate_z_0261(self, bearing, self.center)
